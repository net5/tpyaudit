...
class InstallerWin(InstallerBase):
    ...
    def _check_service(self):
        cc.o(' - check tpyaudit core service status ... ', end='')
        ...
        cc.o(' - check tpyaudit web service status ... ', end='')
...
class InstallerLinux(InstallerBase):
    def __init__(self):
        super().__init__()
        self._def_install_path = '/usr/local/tpyaudit'

    def _check_installation(self):
        cc.o(' - check local installation ... ', end='')

        # old version, the daemon named `eom_ts`.
        # from 2.0.0.1, the daemon rename to `tpyaudit`.
        # we must check both.
        if os.path.exists('/etc/init.d/eom_ts'):
            self._is_installed = True
            self._install_path = '/usr/local/eom/tpyaudit'
            # self._fix_path()
        elif os.path.exists('/etc/init.d/tpyaudit'):
            self._is_installed = True
            self._install_path = '/usr/local/tpyaudit'
            # self._fix_path()
    ...
    def _fix_path(self):
        # self._config_path = '/etc/tpyaudit'
        # self._data_path = os.path.join('/var/lib/tpyaudit')
        # self._log_path = os.path.join('/var/log/tpyaudit')
        self._data_path = os.path.join(self._install_path, 'data')
        self._config_path = os.path.join(self._data_path, 'etc')
        self._log_path = os.path.join(self._data_path, 'log')
    ...
    def _install_service(self):
        ...
        os.symlink('/etc/init.d/tpyaudit', '/etc/rc2.d/S95tpyaudit')
        os.symlink('/etc/init.d/tpyaudit', '/etc/rc3.d/S95tpyaudit')
        os.symlink('/etc/init.d/tpyaudit', '/etc/rc4.d/S95tpyaudit')
        os.symlink('/etc/init.d/tpyaudit', '/etc/rc5.d/S95tpyaudit')

    def _start_service(self):
        cc.v('')
        cc.o('start services...')
        _ret, _ = utils.sys_exec('/etc/init.d/tpyaudit start', direct_output=True)
        if _ret != 0:
            raise RuntimeError('not all services started.')

    def _stop_service(self):
        cc.o(' - stop tpyaudit core service ... ', end='')

        # old version, the daemon named `eom_ts`.
        if os.path.exists('/etc/init.d/eom_ts'):
            utils.sys_exec('/etc/init.d/eom_ts stop')
        # from 2.0.0.1, the daemon rename to `tpyaudit`.
        if os.path.exists('/etc/init.d/tpyaudit'):
            utils.sys_exec('/etc/init.d/tpyaudit stop')
    ...
    def _uninstall_service(self):
        ...
        # from 2.0.0.1, the daemon rename to `tpyaudit`.
        utils.remove('/etc/init.d/tpyaudit')
        utils.remove('/etc/rc2.d/S50tpyaudit')
        utils.remove('/etc/rc3.d/S50tpyaudit')
        utils.remove('/etc/rc4.d/S50tpyaudit')
        utils.remove('/etc/rc5.d/S50tpyaudit')
        # from 3.0.0.3, the start order changed from 50 to 95 for MySQL compatible, because start order of MySQL is 64.
        utils.remove('/etc/rc2.d/S95tpyaudit')
        utils.remove('/etc/rc3.d/S95tpyaudit')
        utils.remove('/etc/rc4.d/S95tpyaudit')
        utils.remove('/etc/rc5.d/S95tpyaudit')

    def _check_service(self):
        cc.v('')
        cc.o('check services status...')
        utils.sys_exec('/etc/init.d/tpyaudit status', direct_output=True)
...
def _main():
    ...
    if env.is_win:
        ...
        raise RuntimeError('Sorry, tpyaudit server does not support this platform yet.')
